...
B = 10e6;  %信号带宽
taup = 20e-6; %信号脉宽
t = linspace(0,taup,taup*fs);          %时间序列 400个点
lfm = exp(1j*pi*B/taup*t.^2);          %LFM信号

SNR=0; %信噪比dB
echo=zeros(data_num,samp_num,3);     %矩阵大小（500,2000,3）
echo_stft=zeros(data_num,100,247,3);  %矩阵大小（500,200,1000,3）
num_label = 5;
...
for m=1:data_num
    ...
    sp1_fft(1:round(2000*((1-Bj/80)/2)))=0; % round用于取整  将1-round(2000*((1-Bj/80)/2))设置为0 即带宽之前设置为0
    sp1_fft(round(2000*((1-Bj/80)/2))+round(Bj/80*2000):2000)=0; % 将带宽之后设置为0
    sp1=ifftshift(ifft(sp1_fft)); % 经过滤波的噪声
    range_tar=1+round(rand(1,1)*1400); % lfm范围起点
